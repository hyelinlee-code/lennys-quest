...
> **⚠️ 2026-07-06 고정 프레임 체계 전환** — 이제 매뉴얼로 만들 것은 **카드 전체가 아니라
> "오벌 내부 초상(인테리어)"** 입니다. 프레임·플라크는 `pipeline/frames/<house>.png`
> 고정 템플릿을 코드가 합성합니다 (`compose_card.py`). 워크플로우:
> 1. `pipeline/manual-prompts/<slug>.txt` 프롬프트를 ChatGPT에 복붙
>    (모티프·포즈·복장·배경·하우스 색이 인물별로 이미 채워져 있음)
> 2. 첨부 2장: 그 인물의 공식 캐리커처 + 스타일 앵커(기존 인테리어 아무거나,
>    예: `pipeline/interiors/dylan-field.png`)
> 3. 결과물을 `pipeline/interiors/<slug>.png`로 저장
> 4. Claude에게 알리면 `python pipeline/compose_card.py <slug> <house>` 합성 + 배포
>
> 아래의 기존 풀카드 템플릿은 **참고용 아카이브**입니다 (프레임 템플릿을 새로 만들 때만 사용).
